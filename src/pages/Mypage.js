...
const Mypage = () => {
  ...
  useEffect(() => {
    ...
    setKg(
      userJson.user1[3].data.flatMap((item) => {
        const key = Object.keys(item)[0];
        const kgValues = item[key]
          .filter((entry) => entry.hasOwnProperty("kg"))
          .map((entry) => entry.kg);
    ...
    setHealth(
      userJson.user1[3].data.flatMap((item) => {
        const key = Object.keys(item)[0];
        const healthValues = item[key]
          .filter((entry) => entry.hasOwnProperty("health"))
        ...
        <div className="area-middle">
          ...
          <div className="area-middle-middle">
            <figure className="profile-card lg-radius">
              <img
                src={profileImg}
                className="profile-img md-radius"
                alt="profile img"
              />
              <div className="about-user">
                ...
                <ul>
                  <li>
                    최근 운동 강도는{" "}
                    <span className="point-1">
                      {userJson.user1[0].profile[2].strength}
                    </span>
                  </li>
                  <li>
                    최근 운동 빈도는{" "}
                    <span className="point-2">
                      {userJson.user1[0].profile[3].frequency}
                    </span>
                  </li>
                  <li>
                    최근 식단 영양은{" "}
                    <span className="point-2">
                      {userJson.user1[0].profile[4].nutrition}
                    </span>
                  </li>
                  <li>
                    최근 식단 기록은{" "}
                    <span className="point-1">
                      {userJson.user1[0].profile[5].record}
                    </span>
                  </li>
                </ul>
              </div>
                ...
                <div className="goal-card lg-radius">
                  <p>
                    ...
                    <span className="gray-2">
                      {userJson.user1[2].goal[0].goalKG}kg
                    </span>
                  </p>
                  <p>
                    기간<span className="gray-2">{until}</span>까지
                  </p>
                </div>
                <div className="goal-card lg-radius">
                  <p>
                    현재<span className="gray-2">{nowKG}kg</span>
                  </p>
                  <p>
                    기간<span className="gray-2">{daysRemaining}일</span>
                    남았습니다
                  </p>
                </div>
              </div>
